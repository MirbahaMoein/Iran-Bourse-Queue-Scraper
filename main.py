...
def get_table(driver):
    ...
    for i in range(5):
        ...
        element['sell'][i] = {}
        try:
            element['buy'][i]['num'] = int(driver.find_element(By.XPATH,   value= '/html/body/div[4]/form/div[3]/div[2]/div[2]/div[2]/div/table/tbody/tr[{}]/td[1]'.format(str(i + 2))).text.replace(',', '').replace(' ', '0'))
            element['buy'][i]['volume'] = int(driver.find_element(By.XPATH, value= '/html/body/div[4]/form/div[3]/div[2]/div[2]/div[2]/div/table/tbody/tr[{}]/td[2]'.format(str(i + 2))).text.replace(',', '').replace(' ', '0'))
            element['buy'][i]['price'] = int(driver.find_element(By.XPATH,  value= '/html/body/div[4]/form/div[3]/div[2]/div[2]/div[2]/div/table/tbody/tr[{}]/td[3]'.format(str(i + 2))).text.replace(',', '').replace(' ', '0'))
            element['sell'][i]['num'] = int(driver.find_element(By.XPATH,   value= '/html/body/div[4]/form/div[3]/div[2]/div[2]/div[2]/div/table/tbody/tr[{}]/td[6]'.format(str(i + 2))).text.replace(',', '').replace(' ', '0'))
            element['sell'][i]['volume'] = int(driver.find_element(By.XPATH, value= '/html/body/div[4]/form/div[3]/div[2]/div[2]/div[2]/div/table/tbody/tr[{}]/td[5]'.format(str(i + 2))).text.replace(',', '').replace(' ', '0'))
            element['sell'][i]['price'] = int(driver.find_element(By.XPATH,  value= '/html/body/div[4]/form/div[3]/div[2]/div[2]/div[2]/div/table/tbody/tr[{}]/td[4]'.format(str(i + 2))).text.replace(',', '').replace(' ', '0'))
        except:
            pass
...
def update_tables():
    ...
    for symbol in main_menu.urls:
        try:
            threading.Thread(target= start_drivers.drivers[symbol].refresh).start()
        except:
            pass
...
def get_url(symbol: str):
    ...
    get_urls.driver.find_element(By.ID, value= 'SearchKey').send_keys(Keys.SPACE)
    get_urls.driver.find_element(By.ID, value= 'SearchKey').send_keys(Keys.BACKSPACE)
    time.sleep(5)
    ...
    for i in range(3):
        ...
        try:
            result = get_urls.driver.find_element(By.CSS_SELECTOR, value= '#SearchResult > div > div.content > table > tbody > tr:nth-child({}) > td:nth-child(1) > a'.format(str(i + 1)))                                                    
        except:
            result = None
        try:
            get_urls.driver.find_element(By.CSS_SELECTOR, value= '#SearchResult > div > div.content > table > tbody > tr:nth-child({}) > td:nth-child(1) > a > span'.format(str(i + 1)))
        except: 
            if result != None:
                if result.text[:len(symbol)] == symbol:
                    ...
                    cursor.execute("INSERT INTO urls (symbol, url) VALUES (?, ?)", (symbol, url))
                    connection.commit()
                    connection.close()
                    return url
    connection.close()
...
def get_urls(symbols: dict):
    ...
    for index in symbols:
        try:
            selection = cursor.execute("SELECT url FROM urls WHERE symbol = ?", (symbols[index],)).fetchall()
    ...
    if needtosearch:
        ...
        time.sleep(5)
        get_urls.driver.find_element(By.XPATH, value= '/html/body/div[3]/div[2]/a[5]').click()
        time.sleep(5)
        for index in symbols:
...
def main_menu():
    ...
    def add_symbol():
        ...
        buybtn = Radiobutton(main_menu.gui, variable = main_menu.side[main_menu.numberofsymbols], value= 'buy')
        sellbtn = Radiobutton(main_menu.gui, variable = main_menu.side[main_menu.numberofsymbols], value= 'sell')
    ...
    def start_updating():
        ...
        for i in range(1, main_menu.numberofsymbols + 1):
            main_menu.symbols[i] = main_menu.symbolfield[i].get('1.0', 'end-1c')
            main_menu.prices[i] = int(main_menu.pricefield[i].get('1.0', 'end-1c'))
